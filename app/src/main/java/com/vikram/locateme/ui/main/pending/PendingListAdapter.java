...
public class PendingListAdapter extends RecyclerView.Adapter<PendingListAdapter.ViewHolder> {
    private List<PendingContact> listPending;
    private final OnItemClickListener listener;
    private Context mContext;

    public interface OnItemClickListener {
        void onItemClick(PendingContact item);
    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(final PendingContact item, final OnItemClickListener listener) {
    ...
    PendingListAdapter(Context context, List<PendingContact> listPending, OnItemClickListener listener) {
    ...
    public void onBindViewHolder(PendingListAdapter.ViewHolder holder, int position) {
        holder.textName.setText(getContactName(mContext, listPending.get(position).getPending()));
        holder.textNumber.setText(listPending.get(position).getPending());
        //get first letter of each String item
        String firstLetter = String.valueOf(getContactName(mContext, listPending.get(position).getPending()).charAt(0));
